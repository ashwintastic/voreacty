...
import { GlobalErrorMessages} from '../config/messages'
 ...
 const ServiceSave = (props) => {
    ...
    if(props.fromPage == "feedback"){
        let thankyouMessage = "THANK YOU"
        let message = GlobalErrorMessages['feedBackSuccess'];
    }
    else if(props.fromPage == "spRegister"){
        let thankyouMessage = "THANK YOU FOR REGISTERING YOUR SERVICE"
        let message = GlobalErrorMessages['serviceSaved'];
    }
    return (
        <div className="">
            <section className="section section-we-made-1">
                <div className="container p-0-xs text-center">
                    <i className="fa fa-check-circle color-green-3 font-250 width-auto m-auto mobile-font-50" aria-hidden="true"></i>
                    <div className="font-26 font-bold color-grey-1">
                        THANK YOU FOR REGISTERING YOUR SERVICE
                    </div>
                    <div className="font-18 p-tb-15">
                        { GlobalErrorMessages.serviceSaved}
                    </div>
